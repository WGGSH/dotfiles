...
module.exports = {
  config: {
    ...
    css: `
      ...
      // ????????????
      .tab_active{
        background: rgba(248,28,229,0.3);
      }
      // ????????????????????????
      .tab_active:hover {
        background: rgba(248,28,229,0.7);
      },
      // ?????????????????????????
      .tab_tab:not(.tab_active):hover{
        background: rgba(248,28,229,0.2);
      }
      .terms_terms{
        background:;
      }
    ...
    backgroundImage: '/home/nao/workspace/git_repos/dotfiles/hyper_back.jpg',
  },
